...
+ (void (^)(AFHTTPRequestOperation *operation, id responseObject))failureBlock:(CatalyzeFailureBlock)failure {
    return ^(AFHTTPRequestOperation *operation, NSError *error) {
        if (failure) {
            NSData *resultData = [NSJSONSerialization dataWithJSONObject:[NSDictionary dictionary] options:0 error:nil];
            if ([operation responseObject] != nil) {
                resultData = [operation responseData];
            }
            failure([NSJSONSerialization JSONObjectWithData:resultData options:0 error:nil], (int)[[operation response] statusCode], error);
        }
    };
}
