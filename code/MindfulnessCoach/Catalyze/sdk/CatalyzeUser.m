...
- (void)logoutWithSuccess:(CatalyzeSuccessBlock)success failure:(CatalyzeFailureBlock)failure {
    [CatalyzeHTTPManager doGet:@"/auth/signout" success:^(id result) {
        ...
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:kCatalyzeAuthorizationTimeoutKey];
...
- (BOOL)isAuthenticated {
    ...
    NSNumber *timeExpires = [[NSUserDefaults standardUserDefaults] objectForKey:kCatalyzeAuthorizationTimeoutKey];
    if (timeExpires && now < [timeExpires doubleValue]) {
        if ([[NSUserDefaults standardUserDefaults] valueForKey:kCatalyzeAuthorizationKey] && ![[[NSUserDefaults standardUserDefaults] valueForKey:kCatalyzeAuthorizationKey] isEqualToString:@""]) {
            return YES;
        }
    }
    return NO;
...
+ (void)logInWithUsernameInBackground:(NSString *)username password:(NSString *)password success:(CatalyzeUserSuccessBlock)success failure:(CatalyzeFailureBlock)failure {
    ...
    [CatalyzeHTTPManager doPost:@"/auth/signin" withParams:body success:^(id result) {
        ...
        if (sessionToken) {
            ...
            timeInSeconds += 82800; // 23 hours
            [[NSUserDefaults standardUserDefaults] setObject:[NSNumber numberWithDouble:timeInSeconds] forKey:kCatalyzeAuthorizationTimeoutKey];
        }
        [[NSUserDefaults standardUserDefaults] synchronize];
        if (success) {
            success(currentUser);
